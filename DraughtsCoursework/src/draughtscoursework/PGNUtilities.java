...
import draughtscoursework.Table.MoveLog;
...
public class PGNUtilities 
{
    ...
    private static final Pattern ATTACK_MOVE = Pattern.compile("^(B|N|R|Q|K)([a-h]|[1-8])?(x)([a-h][0-8])(\\+)?(#)?$");
    private static final Pattern NORMAL_PROMOTION_MOVE = Pattern.compile("(.*?)=(.*?)");
                ...
                if (!line.isEmpty()) 
                {
                    ...
                    else if (isEndOfGame(line))
                    {
                        ...
                        if(!gameText.isEmpty() && gameText.length() > 80) 
                        {
                            ...
                            if(game.isValid()) 
                            {
                                SqlGamePersistence.get().persistGame(game);
                                validCount++;
                            }
                        }
    ...
    {
        final Matcher PromotionMatcher = NORMAL_PROMOTION_MOVE.matcher(pgnText);
        final Matcher plainMajorMatcher = NORMAL_MOVE.matcher(pgnText);
        final Matcher attackMajorMatcher = ATTACK_MOVE.matcher(pgnText);

        int currentCoordinate;
        int destinationCoordinate;

        if(PromotionMatcher.find()) 
        {
            final String destinationSquare = PromotionMatcher.group(1);
    ...
    private static int deriveCurrentCoordinate(final Board board,final String movedPiece,final String destinationSquare,final String disambiguationFile) throws RuntimeException 
    {
        ...
        {
            if (move.getDesCoordinates() == destinationCoordinate && move.getMovedPiece().toString().equals(movedPiece)) 
            {
        ...
        return currentCandidates.size() == 1 ? currentCandidates.iterator().next().getCurrentCoordinates() : extractFurther(currentCandidates, movedPiece, disambiguationFile);
    }
    ...
    private static int extractFurther(final List<Move> candidateMoves,final String movedPiece,final String disambiguationFile) 
    {
        ...
        {
            return currentCandidates.iterator().next().getCurrentCoordinates();
        }
        ...
        {
            final String pos = BoardUtils.INSTANCE.getPositionAtCoordinate(move.getCurrentCoordinates());
        ...
        {
            return candidatesRefined.iterator().next().getCurrentCoordinates();
        }
